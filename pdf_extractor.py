...
# def main():
    # input_file = pymupdf.open(input("Enter your file location here: ").strip().replace('"', '').replace("'", ""))
    # output_file = input("Enter your desired file location and name: ").strip().replace('"', '').replace("'", "")
    
    # for i in range(0, len(list), 1): 
    #     input_file = pymupdf.open(list[i])
    #     output_file = f"pdf_output{i}.txt"
...
# path str parameter - only takes in 1 at a time; for loop is performed at the end
def mainpath(path):
    input_file = pymupdf.open(path)
    output_file = f"extracted_output_{os.path.basename(path)}.txt"
